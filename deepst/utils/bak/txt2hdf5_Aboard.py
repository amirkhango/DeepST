...
rootdir = "D:/Users/xiuwen/Project/TrajectoryMap/Data/32Aboard"
grid_size = 32
...
def load_data_from_COO_fomat(input_path):
    ...
    I=data[:,1] - 1 # x-axis
    J=data[:,2] - 1 # y-axis
    up, down = data[:,3], data[:,4]
    up = sps.coo_matrix((up,(I,J)), shape=(grid_size, grid_size) ).toarray()
    down = sps.coo_matrix((down,(I,J)), shape=(grid_size, grid_size) ).toarray()
...
def load_data(rootdir=rootdir, start='3/1/2015', end='7/1/2015', freq='30Min', year=13):
    ...
    h5 = h5py.File(os.path.join(DATAPATH, 'BJ{}_M{}_T30_Flow_Aboard.h5'.format(year, grid_size)), 'w')
